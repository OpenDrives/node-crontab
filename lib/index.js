...
function CronJob(line, c, m) {
  ...
  this.time = function(){
      ...
      //Not sure why this should be rendered in the @special sytax
      //Adding chech for sunos, which does not support this syntax
      if(process.platform != 'sunos'){
          if (timeIdx >=0 ) {
              time = '@' + keys[timeIdx];
          }
   ...
   */
  this.render = function() {    
    var result = this.time() + ' ' + command.toString();
    if (comment.toString() != '') {
